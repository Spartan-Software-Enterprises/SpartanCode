...
contextBridge.exposeInMainWorld("spartanCode", {
  ...
  importEditorConfig: (projectPath, editors) =>
    ipcRenderer.invoke("editor-config:import", projectPath, editors),
  ...
  runMarketplacePlugin: (manifest, input) =>
    ipcRenderer.invoke("plugins:run", manifest, input),
  ...
  getProtonPassStatus: () => ipcRenderer.invoke("proton-pass:status"),
  getProtonPassVersion: () => ipcRenderer.invoke("proton-pass:version"),
  getProtonPassSecret: (reference) =>
    ipcRenderer.invoke("proton-pass:get", reference),

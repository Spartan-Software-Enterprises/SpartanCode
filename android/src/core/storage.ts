...
import type { ConnectionProfile, MobileSnapshot, QueuedOperation } from "./types";
import {
  isActivity,
  isApproval,
  isArtifact,
...
export async function updateQueuedOperation(
  ...
  update: Partial<Pick<QueuedOperation, "attempts" | "lastError" | "acknowledgedAt">>,
) {
...
export async function clearBridgeToken(endpoint: string) {
  ...
  await AsyncStorage.setItem(
    BRIDGE_TOKEN_INDEX_KEY,
    JSON.stringify((await readBridgeTokenOrigins()).filter((item) => item !== origin)),
  );
}

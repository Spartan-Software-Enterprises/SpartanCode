...
The Android client participates in this contract during bridge refresh and its
bounded continuous sync loop: it
...
When enabled, the loop polls once per minute after success and uses bounded
two-, five-, and ten-minute backoff after failures. It prevents overlapping
requests, stops on unmount, and remains disabled when no endpoint is configured.
...
This is conflict-aware continuous polling, not silent winner selection or
unbounded background transport. Proton Drive remains an encrypted
backup/restore transport and does not receive plaintext artifacts from this
merge layer.
